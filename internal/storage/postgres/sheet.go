...
const (
	QueryStoreSheet = `insert into sheets values(default, $1, 
		(select p.id from processes p
			join users u on u.id = p.user_id where title = $2 and u.email = $3))`

	QueryReadSheet = `select p.title, s.title from sheets s
	...
	QueryReadSheetsContent = `select theme, date, done, sum(AGE(dt_end, dt_start))::varchar as spent from sheets_content sc
	...
	QueryAddRowContent = `insert into sheets_content values (default,
		(select id from sheets where proc_id = 
			(select p.id from processes p
				join users u on u.id = p.user_id and  p.title = $1 and u.email = $2)),
			$3, $4, $5)`

	QueryMarkRowContent = `update sheets_content sc
	set done = true
...
func (pg *pgConnection) StoreSheet(u user.User, s sheet.Sheet, p process.Process) error {
	_, err := pg.conn.Exec(context.Background(), QueryStoreSheet, s.Title, p.Title, u.Email)
	if err != nil {
		log.Println(err)
		return err
	}
...
func (pg *pgConnection) ReadSheet(u user.User, p process.Process) (sheet.Sheet, error) {
	var res sheet.Sheet
	err := pg.conn.QueryRow(context.Background(), QueryReadSheet, p.Title, u.Email).Scan(&res.Process, &res.Title)
	if err != nil {
		log.Println(err)
		return sheet.Sheet{}, err
	}

	rows, err := pg.conn.Query(context.Background(), QueryReadSheetsContent, p.Title, u.Email)
	if err != nil {
		log.Println(err)
		return sheet.Sheet{}, err
	}
...
func (pg *pgConnection) AddRow(u user.User, r sheet.SheetRow, p process.Process) error {
	_, err := pg.conn.Exec(context.Background(), QueryAddRowContent, p.Title, u.Email, r.Theme, r.Date, false)
	if err != nil {
		log.Println(err)
		return err
	}
...
func (pg *pgConnection) MarkRow(u user.User, r sheet.SheetRow, p process.Process) error {
	_, err := pg.conn.Exec(context.Background(), QueryMarkRowContent, p.Title, u.Email, r.Theme)
	if err != nil {
		log.Println(err)
		return err
	}
